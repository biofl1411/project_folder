...
def init_database():
    ...
    # 샘플 데이터가 이미 삽입되었는지 확인하는 플래그 체크
    cursor.execute("SELECT value FROM settings WHERE `key` = 'sample_data_initialized'")
    sample_data_initialized = cursor.fetchone()

    if not sample_data_initialized:
        # 샘플 업체 데이터 (최초 1회만 삽입)
        ...
        cursor.execute('''
            ...
            VALUES ('sample_data_initialized', 'true', '샘플 데이터 초기화 완료 플래그')
